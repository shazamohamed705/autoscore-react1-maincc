...
// Import images
import hallbackImage from '../../assets/ggcar.png';
...
/**
 * Process electrical systems data specifically for electrical inspection points
 * Optimized for better performance and reduced data consumption
 */
const processElectricalData = (data) => {
  if (!data) return null;
  ...
  // Define the electrical points names for filtering
  const electricalPointNames = [
    "إختبار عمل فتحة السقف",
    "بطارية التشغيل 12 فولت",
    "شحن بطارية التشغيل12 فولت",
    "الضوء الأمامي اليمين",
    "الضوء الأمامي اليسار",
    "الضوء الخلفي اليمين",
    "الضوء الخلفي اليسار",
    "اختبار عمل الديفروست الزجاج الخلفي",
    "إختبار عمل النوافذ",
    ...
    "المساحات الأمامية",
    "المساحة الخلفية",
    "اختبار الأنارة الداخلية",
    "إختبار عمل ريموت فتح الأبواب",
    "المرآة اليمين",
    "المرآة اليسار"
  ];

  const electricalService = report.services?.find(service => service.id === 5);
  
  
  
  // Try multiple ways to get electrical points
  let allPoints = [];
  
  if (electricalService?.points) {
    // Method 1: From electrical service (ID 5)
    allPoints = electricalService.points;
  } else if (report.services && report.services.length > 0) {
    // Method 2: Look for electrical service by name
    const electricalByName = report.services.find(service => 
      service.name_ar?.includes("الكهربائية") || 
      service.name_en?.toLowerCase().includes("electrical") ||
      service.name_en?.toLowerCase().includes("systems")
    );
    if (electricalByName?.points) {
      allPoints = electricalByName.points;
    }
  } else if (report.inspection_points && Array.isArray(report.inspection_points)) {
    // Method 3: Filter electrical points from inspection_points
    
    const electricalPoints = report.inspection_points.filter(point => {
      const pointName = point.name_ar || point.point?.name_ar || "";
      const isElectricalPoint = electricalPointNames.includes(pointName);
      if (isElectricalPoint) {
      }
      return isElectricalPoint;
    });
    allPoints = electricalPoints;
  ...
  // Verify we have points from electrical service
  ...
  return {
    ...report,
    serviceInfo: electricalService ? {
      name_ar: electricalService.name_ar,
      name_en: electricalService.name_en,
      description_ar: electricalService.description_ar,
      description_en: electricalService.description_en,
      brief_ar: electricalService.brief_ar,
      brief_en: electricalService.brief_en
    } : {
      name_ar: "المجموعة الكهربائية",
      name_en: "Electrical Systems",
      description_ar: "أوتوسكور يقوم بعمل فحص دقيق و متخصص للأنظمة و الأجزاء  الكهربائية و يرجع الفضل الى الخبراء و المهندسين المختصين و الأجهزة و التقنيات المعتمدة من شركة بوش Bosch الالمانية",
      description_en: "AutoScore performs detailed inspection and assessment for the electrical systems and components , thanks to the experts and BOSCH diagnostic tools"
    },
    electricalPoints: processedPoints,
...
const mapElectricalPoints = (points) => {
  ...
  return points.map((p) => {
    try {
      ...
      console.warn("Error processing electrical point:", err, p);
      return {
        label: p.name_ar || p.name_en || "نقطة غير محددة",
        Stats: "❌",
        info: "",
...
const distributeElectricalPoints = (points) => {
  ...
  points.forEach((point) => {
    ...
    if (nameAr.includes("ضوء") || nameAr.includes("انارة") || nameEn.includes("light")) {
      distributed.lighting.push(point);
    }
    
    if (nameAr.includes("بطارية") || nameAr.includes("شحن") || nameEn.includes("battery")) {
      distributed.battery.push(point);
    }
    
    if (nameAr.includes("راديو") || nameAr.includes("سماعات") || nameAr.includes("زامور") || 
        nameAr.includes("كراسي") || nameAr.includes("ريموت") || nameEn.includes("radio") || 
        nameEn.includes("speaker") || nameEn.includes("horn") || nameEn.includes("seat")) {
      distributed.accessories.push(point);
    }
    
    if (nameAr.includes("نوافذ") || nameAr.includes("سقف") || nameAr.includes("ديفروست") || 
        nameEn.includes("window") || nameEn.includes("sunroof") || nameEn.includes("defrost")) {
      distributed.windows.push(point);
    }
    
    if (nameAr.includes("مرآة") || nameEn.includes("mirror")) {
      distributed.mirrors.push(point);
    }
    
    if (nameAr.includes("مساحة") || nameEn.includes("wiper")) {
      distributed.wipers.push(point);
    }
  });
...
// Main component for electrical systems inspection
const ElectricalSystemsComponent = () => {
  ...
  const filterButtons = useMemo(() => [
    { key: "specifics", label: "جميع نقاط فحص المجموعة الكهربائية" },
    { key: "lighting", label: "أنظمة الإضاءة" },
    { key: "battery", label: "البطارية ونظام الشحن" },
    { key: "accessories", label: "الاكسسوارات والتجهيزات" },
    { key: "windows", label: "النوافذ والسقف" },
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const cacheKey = `electrical_${navigationData?.reportNum || 'default'}`;
      const cachedData = sessionStorage.getItem(cacheKey);
      
      if (cachedData) {
        const parsedData = JSON.parse(cachedData);
        const processedData = processElectricalData(parsedData);
        processElectricalInspectionData(processedData);
        setIsLoading(false);
        return;
      }
      ...
      if (!navigationData?.report && !navigationData?.searchResults) {
        try {
          ...
          if (response.data?.success && response.data?.data) {
            ...
            const processedData = processElectricalData(response.data);
            processElectricalInspectionData(processedData);
            setIsLoading(false);
            return;
          }
      ...
      if (navigationData?.report || navigationData?.searchResults) {
        ...
        if (Array.isArray(navData) && navData.length > 0) {
          ...
          const processedData = processElectricalData(targetReport);
          processElectricalInspectionData(processedData);
        } else if (navData?.data) {
          const processedData = processElectricalData(navData);
          processElectricalInspectionData(processedData);
        } else {
          const processedData = processElectricalData(navData);
          processElectricalInspectionData(processedData);
  ...
  const processElectricalInspectionData = useCallback((processedData) => {
    if (!processedData) {
    ...
    const allPoints = processedData.electricalPoints || [];
    ...
    const serviceDescription = processedData.serviceInfo?.description_ar || 
      "أوتوسكور يقوم بعمل فحص دقيق و متخصص للأنظمة و الأجزاء  الكهربائية و يرجع الفضل الى الخبراء و المهندسين المختصين و الأجهزة و التقنيات المعتمدة من شركة بوش Bosch الالمانية";

    // Distribute points across different filters
    const distributedPoints = distributeElectricalPoints(allPoints);
 
    
    // Create mapped control without useMemo (since it's inside callback)
    const mappedControl = {
      specifics: mapElectricalPoints(distributedPoints.specifics),
      lighting: mapElectricalPoints(distributedPoints.lighting),
      battery: mapElectricalPoints(distributedPoints.battery),
      accessories: mapElectricalPoints(distributedPoints.accessories),
      windows: mapElectricalPoints(distributedPoints.windows),
      mirrors: mapElectricalPoints(distributedPoints.mirrors),
      wipers: mapElectricalPoints(distributedPoints.wipers),
    ...
    const electricalData = {
      itemis: [
        {
          title: processedData.serviceInfo?.name_ar || "المجموعة الكهربائية",
          type: "details",
    ...
    setElectricalData(electricalData);
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600">جاري تحميل بيانات المجموعة الكهربائية...</p>
        </div>
      </div>
    );
  }
            ...
            {({ open }) => (
              <>
                ...
                <Disclosure.Panel className="px-4 py-2 rounded-b-md border border-t-0 border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-200">
                  ...
                  {/* Filter content */}
                  {["specifics", "lighting", "battery", "accessories", "windows", "mirrors", "wipers"].includes(activeFilter) && (
                  (() => {
                      const src = electricalData.itemis[0]?.control[activeFilter] ?? [];
...
export default ElectricalSystemsComponent;